...
static int lua_chat_out(lua_State *state) noexcept {
    int args = lua_gettop(state);
    if( args == 2) {
        const char *string;
        if(lua_isboolean(state,1))
        ...
        chat_out(string, luaL_checknumber(state, 2));
        return 0;
    }
    else return luaL_error(state,"wrong number of arguments in chat_out");
}
...
static int lua_get_key_press(lua_State *state) noexcept {
    int args = lua_gettop(state);
    if(args == 1) {
        if(lua_isnumber(state, 1)) {
            lua_pushboolean(state, (GetKeyState(luaL_checkinteger(state, 1)) & 0x8000));
            return 1;
        }
        else return luaL_error(state,"expected a number in argument 1");
    }
    else return luaL_error(state,"wrong number of arguments in get_key_press");
}
