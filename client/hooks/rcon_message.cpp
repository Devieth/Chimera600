...
static void on_rprint(uint32_t a, uint32_t b, uint32_t c, char **d, uint32_t e) {
    ...
    //Hud_message
    if (strncmp(text,"|nh",3)==0) {
        hud_message(text+3);
        allow = false;
    }
    //Block console spam if console is open
    if (console_is_out()==true) {
        if (strncmp(text,"|t",2)==0 || strncmp(text,"|r",2)==0 || strncmp(text,"|l",2)==0 || strncmp(text,"|c",2)==0 || strncmp(text," ",1)==0) {
            allow = false;
        }
    }
    if (allow) {
